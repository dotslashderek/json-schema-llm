...
| **ABI incompatibility** — `wasm32-unknown-unknown` can't link C-compiled WASM                         | 🟢 Low    | Only matters if we add C dependencies (e.g. `onig` for ECMA regex)                                          | Pure-Rust `regex` crate is sufficient for JSON Schema `pattern`; avoid C deps in core                                                                            |
